# 메인 서버 조직 관리(Org Role Management) — 의존성 통지

- 작성일: 2026-08-03
- 최종 갱신: 2026-08-03 (요청 전건 철회 — 아래 「현재 상태」 참조)
...
**이 문서에 남은 Boards 요청은 없다.** 처음에는 개선 요청 5건으로 작성했으나 검토 과정에서 전건이 철회됐다.

| # | 원래 요청 | 결과 |
|---|---|---|
| 1 | 마스터 목록 읽기 권한 완화 | **철회** — Boards가 자체 경로로 DB를 읽으므로 불필요 |
| 2 | `UserOrgProfileSummary`에 ID 포함 | **철회** — `UserOrgProfiles`를 직접 읽으므로 불필요 |
| 3 | 다건 조직 프로필 조회 경로 | **철회** — `WHERE UserID IN (...)`로 직접 읽으므로 불필요 |
| 4 | 조직 정보 미등록 사용자 정리 | **철회** — 근거 오류(부록 B-1) |
| 5 | 기능 플래그 연동 정책 | **해소** — Boards는 플래그를 읽지 않기로 확정(부록 B-3) |

1~3의 철회 사유는 하나다. 이 요청들은 **"Boards가 메인 서버 API를 호출할 것"이라는 전제** 위에 쓰였는데, 그 전제가 틀렸다. `plugin.API`에 조직 메서드가 없고 `PluginHTTP`는 플러그인 간 호출용이라 **Boards는 api4를 부를 수단 자체가 없다**(부록 B-6). 따라서 api4에 무엇을 추가해도 Boards는 쓰지 못하며, 지금처럼 DB를 직접 읽는다.

**이 문서의 남은 목적은 의존성 통지다** — 아래 「의존 관계」 참조. 메인 서버 팀이 부록 A로 회신한 작업 계획은 기록으로 보존하되, **Boards가 그 작업을 요청하지 않는다.** 필요 여부는 메인 서버가 자체 판단할 사안이다.

## 의존 관계

Boards 플러그인은 메인 서버 소유 테이블 **3개를 읽기 전용으로 직접 조회**한다. 쓰기·마이그레이션은 하지 않는다.

| 테이블 | Boards 사용 |
|---|---|
| `OrgUnits` | 본부·부서 셀렉터, 조상 집합 계산 |
| `PositionDefinitions` | 직책 셀렉터 (`kind='duty'`만), `fullvisibility` 판정 |
| `UserOrgProfiles` | 사용자 소속·직책 판정 (`PrimaryOrgUnitID`·`PrimaryDutyID`) |

**스키마를 바꿀 때 Boards가 깨질 수 있다.** 컬럼 삭제·의미 변경 시 알려주기를 요청한다. 이것이 이 문서의 유일한 실질 요청이다.
...
**Boards는 T1~T3을 쓸 수 없고, 필요하지도 않다.** 따라서 요청을 철회한다.

DB 직접 읽기로 이 기능은 완결된다. 추가 작업이 없고, 권한 맥락 불일치도 발생하지 않으며, `sqlstore/user.go`가 `Users`를 직접 읽는 기존 관례와 같은 계열이다.

`plugin.API`에 조직 조회 메서드를 추가하는 방법도 있으나 **Boards가 요청하지 않는다.** 지금 방식으로 요구가 충족되므로 메인 서버의 API 표면을 넓힐 이유가 없다.

T1~T3을 실제로 만들지 여부는 **메인 서버가 자체 판단할 사안이다.** Boards는 그 판단에 근거를 제공할 위치가 아니며, 이 기능의 범위는 카드 속성 기준 접근 권한에 한정된다.
