...
- [~] T027 [quickstart.md](quickstart.md)의 수동 검증 시나리오 1~6과 회귀 확인표를 배포된 플러그인에서 검증 — **일부만 수행.** 배포본에서 확인한 것: 시나리오 1(그룹 추가 줄로 카드 생성, SC-001), 시나리오 3·4(하위 추가 줄과 ⋯ 메뉴로 하위 카드 생성, SC-002), 그리고 생성·삭제의 실시간 반영. **남은 것**: 시나리오 2(빈 그룹)·5·6, 회귀 확인표 전부(접힌 그룹 · 깊이 한도 · 읽기 전용/권한 · 보드·갤러리·캘린더 보기에 ⋯ 메뉴 항목이 새지 않음 · 기존 `+ 생성`과 그룹 머리글 `+`). SC-004·SC-005는 아직 판정되지 않았다
- [X] T028 브랜치를 `develop`에 선형 병합할 수 있도록 정리 — rebase 불필요했다. `develop`이 브랜치의 조상 그대로라 병합 커밋 없이 fast-forward된다 (constitution 원칙 VIII)

### 정렬 결함 (배포 후 발견, 같은 브랜치에서 수정)

추가 줄의 `+`가 같은 깊이 카드 제목과 어긋났다. 원인 셋이 겹쳤고 각각 실패 테스트를 먼저 썼다.

- [X] A1 레벨별 들여쓰기 — 고정 오프셋 54px이라 모든 깊이의 추가 줄이 한 칸에 겹쳤다. `depth * 22px`로 바꿨다 (커밋 `b8230daa`)
- [X] A2 토글 자리 폭 0 — `.sub-card-indent`·`.expand-toggle-placeholder`의 크기 규칙이 `.TableRow` 아래에만 있어 이 줄에 닿지 않았다. 인라인으로 준다 (커밋 `d176952c`). jsdom은 스타일시트를 적용하지 않아 T-04가 이걸 잡을 수 없었다
- [X] A3 아이콘 자리 미확보 — 제목 앞 아이콘 슬롯 24px과 푸터 셀의 더 넓은 여백 7px이 겹쳐 17px 어긋났다 (커밋 `9e1e782d`)
...
- **Phase 3 테스트**: T009 단독
- **Phase 4 테스트**: T014 → T015 순차 (같은 `tableSubCardRows.test.tsx`)
- **Phase 5 테스트**: T019 → T020 순차 (같은 `tableRow.test.tsx`)
