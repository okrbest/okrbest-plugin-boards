...
- [X] T055 품질 게이트 — `make webapp-ci`(세 단계 따로), `make server-lint`, `make server-test`를 돌려 T001 기준선과 **실패 목록**을 대조한다. **`server-test`는 CI 미집행이라 로컬 출력을 근거로 제시한다**(원칙 I). `git status`에 새 `.scss`가 없는지, `make server-test` 실행 시간이 늘지 않았는지도 본다
- [X] T056 [quickstart.md](./quickstart.md) 11절 완료 판정을 채우고, 게이트·종단 검증·SC 실측을 이 파일 하단에 근거로 남긴다. 검증용 보드는 지운다
...
| 게이트 | 기준선 | 구현 후 | 판정 |
|---|---|---|---|
| jest 실패 스위트 | 57 / 182 | 57 / **184** (**목록 동일**) | 통과 |
| jest 실패 테스트 | 144 / 1119 | 143 / **1168** | 통과 — 새 테스트 49건 전부 통과 |
| tsc 오류 | 23 | 23 (**동일**) | 통과 |
| eslint | 2482 | 2488 | 통과 — 늘어난 6건은 **신규 파일 6개에 붙는 저장소 공통 `Resolve error`**. 실제 지적 0 |
| `make server-test` | 실패 33줄 | 34줄 (**TestPatchBoard 빼면 동일**) | 통과 — CI 미집행, 로컬 출력이 근거 |
| `make server-test` 시간 | 24.13초 | 24.80초 | 통과 |
| `make server-lint` | 지적 11건 | **11건** | 통과 |
| 새 `.scss` 파일 | — | **없음** | 통과 (원칙 II) |
| 신규 파일 라이선스 헤더 | — | 10/10 | 통과 (원칙 VI) |

`TestPatchBoard`는 이 기능과 무관하게 실행마다 흔들린다 (변경 전 2/8, 변경 후 1/8).
근거는 T001 기준선 절에 적었다.
...
| SC | 실측 |
|---|---|
| SC-001 | 요구사항 이미지 12칸 전부 표현. **"C-Level 타 본부는 댓글만"과 "팀원 Task는 본인 것만"이 규칙으로 적힌다** |
| SC-002 | 표 12칸이 규칙 **6줄**로 저장 — 실서버에서 확인 |
| SC-003 | 본부 28개인 팀에서 규칙 6줄. **본부 이름을 쓰는 줄 0** |
| SC-004 | 묶음이 정해진 팀에서 새 보드에 표준 적용 — **표준 버튼 1회** |
| SC-005 | 실보드 `FY27 KKV 업무관리`·`FY27 KKV OKR` 규칙 9줄씩 그대로. 새 필드를 쓰는 줄 0 |
| SC-006 | C-Level에 직책을 더할 때 고치는 자리 **1곳** (팀 묶음). 보드 수와 무관 |
| SC-007 | 어느 묶음에도 없는 직책이 편집기에 저장 전 표시 |
| SC-008 | 팀 관리자 아닌 계정의 묶음 편집 시도 **403** |

### 종단 검증에서 잡은 결함 다섯

게이트만으로는 하나도 안 잡혔다. 전부 배포해서 실제로 만져야 드러난 자리다.

| # | 결함 | 왜 단위 테스트가 못 잡았나 |
|---|---|---|
| 1 | 표 보기 열 제목이 "DIVISION"인 채였다 | 단위 테스트가 헤더 문구를 안 본다 |
| 2 | 팀 설정을 되읽는 길이 없었다 (`GetTeam`이 settings를 안 읽음) | mock이 settings를 담아 돌려주도록 내가 짰다 |
| 3 | OKR을 나중에 켜면 표가 안 떴다 | 첫 렌더 시점에만 굳는 값이라, 한 번에 렌더하는 테스트에서는 안 보인다 |
| 4 | 본부 속성 없는 보드에서 표준이 400으로 거절됐다 | 픽스처에 늘 속성이 있었다 |
| 5 | **부서 관계가 본부 속성을 읽었다** | 픽스처가 두 속성에 같은 값을 넣고 있었다 |

다섯째가 가장 무겁다. 같은 본부면 부서가 달라도 통과해서, 팀장이 남의 부서 Task를
편집할 수 있었다.

### 실보드를 건드린 사고

검증 도중 `FY27 KKV OKR`에 규칙 2줄이 들어갔다(9→11). 백업 후 그 2줄만 지워 9줄로
되돌렸고, 팀의 모든 보드에서 `source=matrix` 줄이 0인 것을 확인했다.

### 계획에서 바뀐 것

- **묶음을 만드는 화면이 계획에 없었다.** 편집기가 묶음 안의 직책만 다루게 설계돼
  있었고, 묶음 자체를 만들 길이 없어 기능 전체가 막혀 있었다 (US4에서 발견)
- **`MatrixContext`가 조직 속성을 하나만 들고 있었다.** 본부와 부서를 갈랐다
- **평가기가 묶음을 무조건 읽지 않는다.** 묶음을 가리키는 규칙이 있을 때만 팀을 읽어,
  기존 보드에서는 조회가 0이다

---

## 배포 후 발견한 결함 — 조직 속성이 배열이다 (2026-08-15)

사용자가 "CSO로 로그인해도 CSO - 영업 카드가 편집되지 않는다"고 보고했다. 추적해 보니
**관계 조건이 실제 저장 형태를 못 읽고 있었다.**

`orgDivision`·`orgDepartment` 속성은 값을 **배열**로 담는다 — `["qqtoq..."]`. 그런데
`cardValue()`가 문자열만 읽어 빈 값을 돌려줬고, 모든 본부·부서 관계가 성립하지 않았다.

```go
// 고치기 전
value, _ := cardRawValue(card, propertyID).(string)   // 배열이면 ""
```

`eachPropertyValueID`로 바꿔 두 형태를 다 읽는다. 카드가 조직을 여럿 담을 수 있으므로
`sameDivision`은 **하나만 겹쳐도** 성립하고, `otherDivision`은 **어느 값도 내 것이
아닐 때만** 성립한다.

**왜 앞선 검증이 못 잡았나.** 테스트 헬퍼가 조직 값을 문자열로 넣고 있었다. T053 종단
검증에서도 카드를 API로 만들며 문자열을 넣었다 — 화면으로 만든 카드만 배열이 된다.
헬퍼를 배열로 고치니 기존 관계 테스트가 전부 깨졌고, 그게 이 결함의 증거다.

**검증** — C-Level 계정(COO, 생산본부)으로 실보드 129장을 측정했다.
...
요구사항의 C-Level 행 그대로다.

### 측정 방법에서 바로잡은 것

처음 측정에서 `curl -i | grep "^token:"`이 빈 문자열을 뽑아 **비인증 요청을 측정**했다.
`/permissions/me`는 비인증에도 200과 함께 `effectivePermission: none`을 돌려주므로
"권한이 없다"로 잘못 읽힌다. `awk`로 헤더를 뽑고 `/api/v4/users/me`로 **누구로
인증됐는지 먼저 확인**한 뒤 측정한다.

`myoungeon.lee@kkv.co.kr`는 현재 `kkv1234!!`를 거부한다(401). 다른 kkv 계정은 통과하고,
비밀번호 변경 시각은 두 계정이 같다 — 이 기능과 무관한 환경 문제다.
