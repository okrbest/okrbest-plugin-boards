...
### D1-R. 개정 — 깊이는 커서 x가 아니라 "어느 목록에 놓이는가"로 정한다

**2026-08-05 수동 검증 후 D1을 뒤집었다.** 사용자 피드백에서 나온 제안이다.

x축 방식은 실제로 쓸 수 없었다. 22px 눈금이 화면에 드러나지 않아 어디까지 밀어야
한 단계 들어가는지 알 수 없고, 이 표는 제목 셀이 좁아 조작 폭도 부족하다. 노션은
항목 폭이 넓고 들여쓰기가 커서 통하지만 여기서는 아니다.

**개정 규칙**: 경계 바로 아래 행이 속한 목록에 끼워 넣는다. 하위 카드 앞에 놓으면
그 카드의 형제가 되고, 최상위 카드 앞에 놓으면 최상위가 된다. 하위 목록의 마지막
자리는 이미 화면에 있는 `+ 새 하위 카드` 줄이 경계를 잡아준다.

**하위로 넣으려면 그 부모가 펼쳐져 있어야 한다.** 접힌 부모의 하위 행은 화면에
없으므로 경계 자체가 생기지 않는다. 노션과 같은 제약이고, 이 덕분에 여러 단계가
겹쳐도 눈에 보이는 대로 동작한다 — x축 방식이 3단 이상에서 애매해지던 문제가
사라진다.

사용자가 이미 보고 있는 경계를 그대로 쓰므로 배울 눈금이 없다.
